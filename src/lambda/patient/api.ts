...
export const getAllPatients = async (event: APIGatewayEvent) => {
  ...
  const { expressionAttributeNames, finalFieldsToReturn } =
    convertToExpressionAttributeNames(fieldsToReturn);
  const response: ScanOutput = await scanTable(
    process.env.PATIENT_TABLE || "test-table",
    "isDeleted = :isDeleted",
    { ":isDeleted": false } as unknown as ExpressionAttributeValueMap,
...
export const getPatientById = async (event: APIGatewayEvent) => {
  ...
  const response: QueryOutput = await query(
    process.env.PATIENT_TABLE as string,
  ...
  if (!response.Items?.length) {
    throw new Error(
      JSON.stringify({ statusCode: 404, error: "Patient not found with id " + patientId }),
    );
  }
...
export const updatePatient = async (event: APIGatewayEvent) => {
  ...
  const response: UpdateItemOutput = await update(
    process.env.PATIENT_TABLE as string,
    { id: patientId },
    payload,
  );
